window.addEventListener('DOMContentLoaded', () => {
    ...
    class MenuCard {
        ...
        render() { //render() -классическое название метода, чтобы сформировать верстку
            ...
            if (this.classes.length === 0) { //если не был задан ни один класс, то задаем всегда menu__item
                this.element = 'menu__item';
                element.classList.add(this.element);
            } else {
                this.classes.forEach(className => element.classList.add(className)); //присваем класс menu__item div. Избавляемся от лишнего div
            }
    ...
    new MenuCard( //упрощенная запись. Альтернатива приведенному выше
        ...
        9, //9$
        '.menu .container',
        //'menu__item',  //теперь мы можем добавлять сколько угодно классов
        //'big'
    ).render();
